...
specified_tgts = [
  ...
  "index.html",
  "src/bin/e.bash.txt",
  "src/bin/launch.bash.txt"
]

bin = "#{$env.src_home_path}/bin"
images = filenames_in_dir_named "images"
tgt_images = images.map {|e| "img/#{e}"}
articles_dirs_names = ["our_articles", "others_articles"]
other_public_dirs = [ "img", "src", "src/bin", "src/left",
  "src/right", "src/our_articles", "src/others_articles"
]
tgt_dirs = ["../public"] +
...
new_entry "tgts:#{tgt_dirs_string} #{tgts_string} src_links"
tab "ruby tell_photo_width.rb"



new_entry "src_links: list Makefile"
tab "while read f; do \\"
tab "  rm -rf ../public/src/$$f.txt; \\"
tab "  ln $$f ../public/src/$$f.txt; \\"
tab "done <list"
tab "touch $@"
...
["e", "launch"].each do |f|
  new_entry "../public/src/bin/#{f}.bash.txt: #{bin}/#{f}"
  tab "rm -rf $@"
  tab "ln $? $@"
end
